...
export default function AllProducts() {
  ...
  const { setElectronics, getElectronics } = useData();
  ...
  useEffect(() => {
    const fetchElectronics = async () => {
      setLoading(true);
      const ElectronicsData = await getElectronics();
      setElectronics(ElectronicsData);
      setAllProducts(ElectronicsData);
      setVisibleProducts(ElectronicsData.slice(0, productsPerPage)); // Load initial 30 products
      setCount(ElectronicsData.length);
      setLoading(false);
    };
